...
1. 打开 [BepInEx 下载页](https://github.com/BepInEx/BepInEx/releases)（选最新版本，当前为 5.4.23.5）
2. 下载 **`BepInEx_win_x64_*.zip`**（Windows x64 版本）
3. 解压这个 zip，你会得到：
   - `BepInEx/` 文件夹
   - `dotnet/` 文件夹（.NET 运行时，IL2CPP 必需）
   - `doorstop_config.ini`
   - `winhttp.dll`
...
1. Go to [BepInEx releases](https://github.com/BepInEx/BepInEx/releases) (get the latest version, currently 5.4.23.5)
2. Download **`BepInEx_win_x64_*.zip`** (Windows x64)
3. Extract the zip. You should see:
   - `BepInEx/` folder
   - `dotnet/` folder (.NET runtime, required for IL2CPP)
   - `doorstop_config.ini`
   - `winhttp.dll`
