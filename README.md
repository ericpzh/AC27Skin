...
1. 将下载的 `AC27Skin.zip` 解压
2. 把 `AC27Skin.dll` 复制到 `BepInEx\plugins\` 目录下
3. 把 `overrides/` 文件夹复制到游戏根目录
3. 最终结构如下：
...
1. Extract the downloaded `AC27Skin.zip`
2. Copy `AC27Skin.dll` into `BepInEx\plugins\`
3. Copy the `overrides/` folder into the game root
